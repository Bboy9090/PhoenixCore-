        ...
        if os.path.exists(mp):
            ...
            try:
                ...
                for app in apps:
                    ...
                    print(
                        f'  - Verified Config for app: {name} [Compat: {compat_layer}] [Sandbox: {sandbox.get("enabled", False)}]'
                    )
                    if app.get("placeholder", False) is True:
                        print(
                            f'[-] Error: Placeholder application "{name}" is prohibited by governance rules.'
                        )
                        sys.exit(1)
