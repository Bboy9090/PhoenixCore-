...
def check_release_checklist(pr_ref, pr_title):
    ...
    requirements = [
        ...
        (
            "[ ] Verification Screenshots / Visual Proof",
            ["screenshot", "image", "png", "jpg", "visual"],
        ),
    ]
    ...
    if is_release:
        print(
            "[!] Release branch/title detected. Enforcing strict verification requirements."
        )
        missing = []
        ...
        if missing:
            ...
            print(
                "    You must supply the following verification outputs in your PR description:"
            )
            for m in missing:
                print(f"    * {m}")
            sys.exit(1)
        print(
            "[+] PR body contains required validation hashes, logs, and screenshots references."
        )
    else:
