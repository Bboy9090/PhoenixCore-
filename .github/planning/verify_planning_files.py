...
def test_markdown_parsing():
    ...
    try:
        ...
        print(
            f"[+] Local verification passed for Markdown parsing. Found {len(epics)} epics."
        )
        ...
        for i, epic in enumerate(epics):
            ...
            print(
                f"    - Epic {i+1} \"{epic['title']}\": {num_issues} child issues parsed."
            )
            if not (3 <= num_issues <= 7):
                print(
                    f"[-] Expected between 3 and 7 child issues for epic, found {num_issues}"
                )
                sys.exit(1)
    except Exception as e:
...
if __name__ == "__main__":
    ...
    print(
        "[+] All verification checks completed successfully! Ready for remote deployment."
    )
