...
def parse_markdown_epics(filepath):
    ...
    for block in epic_blocks[1:]:
        ...
        epic_labels = [l.strip().strip("`") for l in labels_match.group(1).split(",")] if labels_match else []
        ...
        for issue_block in issue_blocks[1:]:
            ...
            dod_match = re.search(r"-\s*\*\*Definition of Done:\*\*\s*(.*)", issue_block)
            val_match = re.search(r"-\s*\*\*Validation Commands:\*\*\s*```bash\s*(.*?)\s*```", issue_block, re.DOTALL)
            dep_match = re.search(r"-\s*\*\*Blocked-by Dependencies:\*\*\s*(.*)", issue_block)
...
def main():
                ...
                if all_ms:
                    for m in all_ms:
                        if m['title'] == ms['title']:
                            ...
                            print(f"[~] Found existing milestone {ms['title']} (Number: {m['number']})")
                            break
            elif res:
    ...
    if os.path.exists(epics_file):
        ...
        for epic in epics:
            ...
            epic_issue, _ = make_github_request(f"{base_url}/issues", token, epic_payload, method="POST")
            ...
            if epic_issue:
                ...
                for child in epic.get("child_issues", []):
                    ...
                    child_issue, _ = make_github_request(f"{base_url}/issues", token, child_payload, method="POST")
                    if child_issue:
