...
pub fn start_build(
    ...
    manager.start_build(PathBuf::from(build_dir))?;
    Ok(manager.get_status())
}

/// Tauri command: Get build status
#[tauri::command]
pub fn get_build_status(state: State<'_, Arc<Mutex<BuildManager>>>) -> BuildStatus {
    let manager = state.lock().unwrap();
    manager.get_status()
}

/// Tauri command: Pause build
#[tauri::command]
pub fn pause_build(state: State<'_, Arc<Mutex<BuildManager>>>) -> Result<(), String> {
    let mut manager = state.lock().unwrap();
    manager.pause_build()
}

/// Tauri command: Resume build
#[tauri::command]
pub fn resume_build(state: State<'_, Arc<Mutex<BuildManager>>>) -> Result<(), String> {
    let mut manager = state.lock().unwrap();
    manager.resume_build()
}

/// Tauri command: Cancel build
#[tauri::command]
pub fn cancel_build(state: State<'_, Arc<Mutex<BuildManager>>>) -> Result<(), String> {
    let mut manager = state.lock().unwrap();
    manager.cancel_build()
}

/// Tauri command: Get build logs
#[tauri::command]
pub fn get_build_logs(state: State<'_, Arc<Mutex<BuildManager>>>) -> Result<Vec<LogEntry>, String> {
    let manager = state.lock().unwrap();
    manager.get_logs()
}
