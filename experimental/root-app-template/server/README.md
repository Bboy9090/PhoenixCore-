## Phoenix Core note (repository fork)

This file documents the **root Expo / tRPC / Drizzle template** under `server/_core/`. It is **not** the Phoenix Core USB API.



For Phoenix Core REST (devices, safety, USB builds), use **`backend/` (FastAPI)** — see `docs/CANONICAL_RUNTIME.md` and `docs/AUDIT_SECOND_PASS_STRUCTURE.md`.



The legacy Flask app `server/api.py` is **deprecated** in favor of `backend/main.py`.
