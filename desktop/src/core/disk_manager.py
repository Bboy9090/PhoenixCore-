...
class DiskWriter(QThread):
    ...
    def _unmount_device(self):
        ...
        try:
            ...
            if system == "Linux":
                ...
                # Use diskutil to unmount with force to override spotlight indexing locks (mds)
                subprocess.run(['diskutil', 'unmountDisk', 'force', self.target_device],
                             capture_output=True, check=False)
                self.logger.info(f"Force unmounted {self.target_device}")
    ...
    def _write_image_data(self, total_size: int) -> bool:
        ...
        try:
            ...
            target_path = self.target_device
            if platform.system() == "Darwin" and target_path.startswith("/dev/disk"):
                target_path = target_path.replace("/dev/disk", "/dev/rdisk")
                    ...
                    if result.returncode == 0:
                        ...
                        for line in output.split('\n'):
                            if line.strip().startswith('/dev/disk'):
                                ...
                                current_device = {
                                    'path': dev_path,
                                    'model': 'External Physical Disk',
                                    'size': 0,
                                    'is_removable': True,
                                    'vendor': 'External'
                                }
                                drives.append(current_device)
                                ...
                                if match:
                                    ...
                                    if unit == 'TB':
                                        current_device['size'] = int(val * 1024**4)
                                    elif unit == 'MB':
                                        current_device['size'] = int(val * 1024**2)
                                    else:
                                        current_device['size'] = int(val * 1024**3)
                except Exception as e:
                    self.logger.debug(f"Error querying physical drives on macOS: {e}")
