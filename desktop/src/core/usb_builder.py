...
class StorageBuilder(QThread):
    ...
    def __init__(self, safety_level: SafetyLevel = SafetyLevel.STANDARD, dry_run: bool = False):
        super().__init__()
        ...
        self.mock_disconnect = False
        # _build_successful flag will be created upon successful build
    ...
    def start_multiboot_build(self, recipe: DeploymentRecipe, target_device: str,
                             hardware_profile: HardwareProfile, source_files: Dict[str, str],
                             grub_config, dry_run: bool = False):
    ...
    def run(self):
        ...
        try:
            ...
            if getattr(self, 'mock_disconnect', False):
                raise IOError("Target device simulated disconnect detected")
            ...
            if self.dry_run:
                self._write_dry_run_audit_event()
            ...
            # Perform rollback if needed and if any rollback operations were registered
            if (self.is_cancelled or not hasattr(self, '_build_successful')) and self.rollback_operations:
    ...
    def _create_partitions_linux(self) -> bool:
        ...
        try:
            ...
            self._run_command_safe(['sudo', 'partprobe', self.target_device],
                         capture_output=True, text=True)
    ...
    def _create_partitions_macos(self) -> bool:
        ...
        try:
            ...
            self._run_command_safe(['diskutil', 'unmountDisk', 'force', disk_id],
                         capture_output=True, text=True)
            ...
            result = self._run_command_safe(partition_cmd, capture_output=True, text=True)
    ...
    def _create_partitions_windows(self) -> bool:
        ...
        try:
            ...
            self._write_text_safe(script_path, '\n'.join(diskpart_script))
    ...
    def _format_partitions(self) -> bool:
        ...
        try:
            ...
            for i, partition in enumerate(self.recipe.partitions, 1):
                ...
                if not self.dry_run and not os.path.exists(partition_device):
    ...
    def _format_partition(self, device: str, partition: PartitionInfo) -> bool:
        ...
        try:
            ...
            if system == "Linux":
                ...
                result = self._run_command_safe(cmd, capture_output=True, text=True)
                if result.returncode != 0:
                ...
                self._add_rollback_operation(
                    lambda dev=device: self._run_command_safe(
                        ['sudo', 'wipefs', '-a', dev],
                        capture_output=True, check=False
                    )
                )
    ...
    def _mount_partitions(self) -> Dict[str, str]:
        ...
        try:
            ...
            if self.dry_run:
                self._log_message("INFO", "[DRY RUN] Simulating partition mounting")
                for i, partition in enumerate(self.recipe.partitions, 1):
                    ...
                    self._log_message("INFO", f"[DRY RUN] Mounted {partition.name} (partition {i}) at {mount_point}")

                    self._add_rollback_operation(
                        lambda mp=str(mount_point): self._log_message("INFO", f"[DRY RUN] Simulating unmount for {mp}")
                    )
                return mount_points
            ...
            for i, partition in enumerate(self.recipe.partitions, 1):
                ...
                if platform.system() == "Linux":
                    ...
                    if result.returncode == 0:
                        self._add_rollback_operation(
                            lambda mp=str(mount_point): self._run_command_safe(
                                ['sudo', 'umount', mp],
                                capture_output=True, check=False
                            )
                        )
    ...
    def _deploy_files(self, mount_points: Dict[str, str]) -> bool:
        ...
        try:
            ...
            if self.dry_run:
                self._log_message("INFO", f"[DRY RUN] Simulating file deployment for deployment type: {self.recipe.deployment_type.value}")
                # Simulate realistic wait time for staging copies
                time.sleep(0.1)
                return True
...
'''
    ...
    def _configure_bootloader(self, mount_points: Dict[str, str]) -> bool:
        ...
        try:
            ...
            if self.dry_run:
                self._log_message("INFO", "[DRY RUN] Simulating bootloader configuration: SUCCESS")
                return True
    ...
    def _finalize_build(self, mount_points: Dict[str, str]) -> bool:
        ...
        try:
            ...
            if metadata_partition:
                ...
                self._write_text_safe(metadata_file, json.dumps(metadata, indent=2))
            ...
            # Sync filesystem
            if platform.system() == "Linux":
                self._run_command_safe(['sync'], check_cancellation=False)
    ...
    def _unmount_device_partitions(self):
        ...
        try:
            if self.dry_run:
                self._log_message("INFO", "[DRY RUN] Simulating partition unmounting for target device")
                return
                ...
                for line in result.stdout.strip().split('\n'):
                    if line.strip():
                        ...
                        if len(parts) >= 2 and parts[1] != '':  # Has mount point
                            ...
                            self._run_command_safe(['sudo', 'umount', device],
                                         capture_output=True, check=False)
    ...
    def _configure_multiboot_grub(self, partition_mounts: Dict[str, str]) -> bool:
        ...
        try:
            ...
            if self.dry_run:
                self._log_message("INFO", "[DRY RUN] Simulating GRUB multi-boot configuration: SUCCESS")
                return True
    ...
    def _run_command_safe(self, cmd_args: List[str], check_cancellation: bool = True, **kwargs) -> subprocess.CompletedProcess:
        """
        Orchestrates secure subprocess executions, mock-intercepting operations in dry_run mode.
        """
        # Periodic check for interruption/disconnect
        if check_cancellation:
            if self.is_cancelled:
                raise RuntimeError("Build cancelled by user")
            if getattr(self, 'mock_disconnect', False):
                raise IOError("Target device simulated disconnect detected")
            if not self.dry_run and self.target_device and not os.path.exists(self.target_device):
                raise IOError("Target device physically disconnected")

        # Convert list elements to strings
        cmd_str_list = [str(x) for x in cmd_args] if isinstance(cmd_args, list) else [cmd_args]
        cmd_name = cmd_str_list[0] if cmd_str_list else ""

        if self.dry_run:
            self._log_message("INFO", f"[DRY RUN] Simulating command execution: {' '.join(cmd_str_list)}")

            # Simulate slight delay to mimic disk write operation speed
            time.sleep(0.05)

            # Generate simulated command outputs if requested
            stdout_data = ""
            stderr_data = ""
            returncode = 0

            # Custom mock command handling for system inspection commands
            if cmd_name == "lsblk":
                # Simulated lsblk list output for the target device
                device_name = self.target_device.split('/')[-1] if '/' in self.target_device else self.target_device
                stdout_data = f"{device_name} 0\n"
                if self.recipe:
                    for i, partition in enumerate(self.recipe.partitions, 1):
                        part_name = f"{device_name}1" if i == 1 else f"{device_name}{i}"
                        stdout_data += f"└─{part_name} /media/partition_{i}\n"
            elif cmd_name == "powershell" or "Get-Partition" in "".join(cmd_str_list):
                # Simulated PowerShell Get-Partition JSON output
                partitions_json = []
                if self.recipe:
                    for i, partition in enumerate(self.recipe.partitions, 1):
                        partitions_json.append({
                            "PartitionNumber": i,
                            "DriveLetter": chr(68 + i),  # E, F, etc.
                            "Size": partition.size_mb * 1024 * 1024 if partition.size_mb != -1 else 10 * 1024 * 1024 * 1024
                        })
                stdout_data = json.dumps(partitions_json)
            ...
            # Physical execution in non-dry-run mode
            return subprocess.run(cmd_args, **kwargs)

    def _copy_file_safe(self, src: Any, dst: Any):
        """Safely mock or execute single file copies"""
        if self.is_cancelled:
            raise RuntimeError("Build cancelled by user")
        if getattr(self, 'mock_disconnect', False):
            raise IOError("Target device simulated disconnect detected")
        ...
        if self.dry_run:
            self._log_message("INFO", f"[DRY RUN] Simulating copy: {src_str} -> {dst_str}")
            time.sleep(0.01)
            return True
        else:
            return shutil.copy2(src, dst)

    def _copytree_safe(self, src: Any, dst: Any, **kwargs):
        """Safely mock or execute directory copy trees"""
        if self.is_cancelled:
            raise RuntimeError("Build cancelled by user")
        if getattr(self, 'mock_disconnect', False):
            raise IOError("Target device simulated disconnect detected")
        ...
        if self.dry_run:
            self._log_message("INFO", f"[DRY RUN] Simulating directory copy: {src_str} -> {dst_str}")
            time.sleep(0.02)
            return True
        else:
            return shutil.copytree(src, dst, **kwargs)

    def _write_text_safe(self, file_path: Any, content: str):
        """Safely mock or write file contents"""
        if self.is_cancelled:
            raise RuntimeError("Build cancelled by user")
        if getattr(self, 'mock_disconnect', False):
            raise IOError("Target device simulated disconnect detected")
        ...
        if self.dry_run:
            self._log_message("INFO", f"[DRY RUN] Simulating write file: {path_str}")
            return True
        else:
    ...
    def _extract_iso_safe(self, iso_path: str, dest_dir: str) -> bool:
        """Safely mock or execute ISO extractions"""
        if self.is_cancelled:
            raise RuntimeError("Build cancelled by user")
        if getattr(self, 'mock_disconnect', False):
            raise IOError("Target device simulated disconnect detected")
        ...
        if self.dry_run:
            self._log_message("INFO", f"[DRY RUN] Simulating extraction of ISO: {iso_path} to {dest_dir}")
            return True
        else:
            return self._extract_iso(iso_path, dest_dir)

    def _write_dry_run_audit_event(self):
        """Register structured forensic trail events to active JSONL logs under verdict state 'DRYRUN_SIMULATION'"""
        try:
            from src.core.safety_validator import get_audit_log_path
            log_path = get_audit_log_path()
            log_path.parent.mkdir(parents=True, exist_ok=True)

            audit_event = {
                "timestamp": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
                "device_path": self.target_device,
                "verdict": "DRYRUN_SIMULATION",
                "severity": "SAFETY_INFO",
                "confidence_score": 100,
                "recipe_name": self.recipe.name if self.recipe else "Unknown",
                "hardware_profile": self.hardware_profile.model if self.hardware_profile else "Unknown",
                "operator_message": f"Simulated dry-run boot-media deployment on {self.target_device}",
                "audit_persistence": "SUCCESS"
            }

            with open(log_path, 'a', encoding='utf-8') as f:
                f.write(json.dumps(audit_event) + "\n")

            self._log_message("INFO", f"Registered dry-run simulation audit event to {log_path}")
        except Exception as e:
            self._log_message("WARNING", f"Failed to persist dry-run audit record: {e}")
