...
def validate_matrix(matrix: dict[str, Any]) -> dict[str, Any]:
    ...
    for name, record in products.items():
        ...
        require(
            isinstance(record.get("owns"), list) and record["owns"],
            f"{name} owns list missing",
        )
        require(
            isinstance(record.get("does_not_own"), list) and record["does_not_own"],
            f"{name} does_not_own list missing",
        )
        require(
            not set(record["owns"]) & set(record["does_not_own"]),
            f"{name} ownership overlap",
        )
    ...
    require(
        isinstance(matrix.get("supported_foundation"), list)
        and matrix["supported_foundation"],
        "supported foundation missing",
    )
...
def validate_docs(repo_root: Path) -> None:
    ...
    boundaries = (repo_root / "docs" / "PRODUCT_BOUNDARIES.md").read_text(
        encoding="utf-8"
    )
    required_readme = [
