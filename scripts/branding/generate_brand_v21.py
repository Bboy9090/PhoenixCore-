...
WALLS=[{"width":1920,"height":1080},{"width":2560,"height":1440},{"width":3840,"height":2160}]
...
def main():
 ...
 out=Path(a.output); data=json.loads(Path(a.manifest).read_text()); (out/'icons').mkdir(parents=True,exist_ok=True); (out/'wallpapers').mkdir(parents=True,exist_ok=True)
 icons=[]; walls=[]
 for p in data['products']:
  slug=p['slug']; (out/'icons'/f'{slug}.svg').write_text(icon(p)+'\n'); (out/'wallpapers'/f'{slug}-hero.svg').write_text(wallpaper(p)+'\n'); icons.append({'product':p['name'],'path':f'{out.as_posix()}/icons/{slug}.svg','role':p['role']}); walls.append({'product':p['name'],'path':f'{out.as_posix()}/wallpapers/{slug}-hero.svg','scene':p['scene']})
 spec={'schema_version':'2.1.0','icon_master':{'format':'svg','view_box':'0 0 512 512','required_png_exports':SIZES,'alpha_required_for_symbol_exports':True},'hero_wallpaper':{'format':'svg','view_box':'0 0 1920 1080','required_png_exports':WALLS},'quality_gates':['valid XML','no embedded raster images','no baked labels or UI fragments','no fake checkerboard backgrounds','one distinct silhouette per application','small-size silhouette remains legible']}
 (out/'EXPORT_SPEC.json').write_text(json.dumps(spec,indent=2)+'\n'); (out/'ASSET_MANIFEST.json').write_text(json.dumps({'schema_version':'2.1.0','icons':icons},indent=2)+'\n'); (out/'WALLPAPER_MANIFEST.json').write_text(json.dumps({'schema_version':'2.1.0','canvas':{'width':1920,'height':1080,'format':'svg'},'wallpapers':walls},indent=2)+'\n')
 print(json.dumps({'products':len(data['products']),'icons':len(icons),'wallpapers':len(walls),'output':str(out)},indent=2))
