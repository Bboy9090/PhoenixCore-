...
class RealWriterRequest:
    ...
    def __init__(self, **kwargs):
        ...
        self.request_id = (
            kwargs.get("request_id") or f"req_{str(uuid.uuid4())[:32].replace('-', '')}"
        )
...
class RealWriterResult:
    ...
    def __init__(self, **kwargs):
        ...
        self.destructive_operations_enabled = kwargs.get(
...
class NullDisabledWriterAdapter(PlatformWriterAdapter):
    ...
    def execute_write(self, request: RealWriterRequest) -> RealWriterResult:
        return RealWriterResult(
            ...
            blocked=True,
            block_reasons=[
                "Real writing remains disabled by default. No active platform adapter configured."
            ],
...
class WindowsLabWriterAdapter(PlatformWriterAdapter):
    ...
    def execute_write(self, request: RealWriterRequest) -> RealWriterResult:
        return RealWriterResult(
            ...
            blocked=True,
            block_reasons=[
                "Physical USB writing is blocked on Windows in this phase. Use file-backed writer instead."
            ],
...
class MacOSLabWriterAdapter(PlatformWriterAdapter):
    ...
    def execute_write(self, request: RealWriterRequest) -> RealWriterResult:
        return RealWriterResult(
            ...
            blocked=True,
            block_reasons=[
                "Physical USB writing is blocked on macOS. Use file-backed writer instead."
            ],
            next_required_action="use_file_backed_fallback_writer",
        )
...
class LinuxLabWriterAdapter(PlatformWriterAdapter):
    ...
    def execute_write(self, request: RealWriterRequest) -> RealWriterResult:
        return RealWriterResult(
            ...
            blocked=True,
            block_reasons=[
                "Physical USB writing is blocked on Linux. Use file-backed writer instead."
            ],
            next_required_action="use_file_backed_fallback_writer",
        )
...
class FileBackedLabWriterAdapter(PlatformWriterAdapter):
    ...
    def execute_write(self, request: RealWriterRequest) -> RealWriterResult:
        ...
        try:
            ...
            verification_passed = (
                (written_sha == expected_sha) if expected_sha else True
            )
...
def build_removable_target_identity_lock(
    target_drive: str, scan_payload: dict = None
) -> dict:
    """
    ...
    device_path = (
        (
            scanner_device.get("drive_path")
            or scanner_device.get("path")
            or scanner_device.get("drive")
            or ""
        )
        if scanner_device
        else ""
    ...
    if scanner_device and device_path:
        ...
        is_removable = (
            scanner_device.get("is_removable")
            or scanner_device.get("removable")
            or False
        )
        is_external = (
            scanner_device.get("is_external") or scanner_device.get("external") or False
        )
        is_fixed = (
            scanner_device.get("is_fixed") or scanner_device.get("fixed") or False
        )
        is_system = (
            scanner_device.get("is_system")
            or scanner_device.get("is_system_drive")
            or False
        )
        size_bytes = (
            scanner_device.get("size_bytes")
            or scanner_device.get("capacity_bytes")
            or 0
        )
        confidence = scanner_device.get("confidence", "low")
        detection_source = scanner_device.get("detection_source")
        bus_protocol = scanner_device.get("bus_protocol") or scanner_device.get(
            "bus_type"
        )
        ...
        if is_v2_device:
            ...
            dev_hash = scanner_device.get("device_identity_hash") or scanner_device.get(
                "identity_hash"
            )
        volume_label = (
            scanner_device.get("volume_label")
            or scanner_device.get("display_name")
            or scanner_device.get("label")
        )
    else:
        stable_id = None
        serial = None
        ...
        warnings_from_scanner = [
            "No scanner v2 device matched; using fallback identity."
        ]
    ...
    if is_fixed is True:
        if (
            "Target drive is fixed/internal." not in block_reasons
            and "Drive is fixed/internal, not removable." not in block_reasons
        ):
            block_reasons.append("Target drive is fixed/internal.")
    if is_system is True:
        if (
            "Target drive is flagged as system drive." not in block_reasons
            and "Drive is a system/boot drive." not in block_reasons
        ):
    ...
    lock_record = {
        ...
        "next_required_action": (
            "verify_target_identity" if not blocked else "resolve_preflight_blockers"
        ),
    }
...
def rescan_and_compare_target_identity(
    identity_lock: dict, latest_scan_payload: dict = None
) -> dict:
    """
    ...
    if not identity_lock or identity_lock.get("blocked"):
        return {
            "match": False,
            "drift_detected": True,
            "error": "Invalid or blocked identity lock payload.",
        }
    ...
    if scanner_device and scanner_device.get("drive_path"):
        ...
        if not latest_drive:
            return {
                "match": False,
                "drift_detected": True,
                "error": "Target drive was not found during re-scan.",
            }
        latest_hash = latest_drive.get("device_identity_hash") or latest_drive.get(
            "identity_hash"
        )
    ...
    return {
        ...
        "error": (
            None if match else "Target drive identity has changed since initialization."
        ),
    }


def build_physical_writer_preflight_result(
    identity_lock: dict, image_payload: dict = None, readiness_gate: dict = None
) -> dict:
    """
    ...
    if not lock_ok:
        block_reasons.extend(
            identity_lock.get("block_reasons", ["Identity lock failed verification."])
        )
    ...
    if image_payload:
        ...
        image_size_bytes = (
            image_payload.get("image_size_bytes")
            or image_payload.get("size_bytes")
            or 0
        )
        if not image_sha256:
    ...
    scanner_confidence = (
        identity_lock.get("confidence", "low") if identity_lock else "low"
    )
    scanner_detection_source = (
        identity_lock.get("detection_source") if identity_lock else None
    )
    ...
    scanner_block_reasons = (
        list(identity_lock.get("block_reasons", [])) if identity_lock else []
    )
    scanner_warnings = list(identity_lock.get("warnings", [])) if identity_lock else []

    if scanner_confidence == "low":
        block_reasons.append(
            "Scanner confidence is low; identity lock is unreliable for lab write eligibility."
        )

    block_reasons.append(
        "Physical USB writing remains locked. Phase 5A-2 preflight mode only."
    )
    ...
    preflight = {
        ...
        "target_identity_hash": (
            identity_lock.get("device_identity_hash") if identity_lock else None
        ),
        "target_volume_label": (
            identity_lock.get("volume_label") if identity_lock else None
        ),
        ...
        "target_is_removable": (
            identity_lock.get("is_removable") if identity_lock else False
        ),
        "target_is_external": (
            identity_lock.get("is_external") if identity_lock else False
        ),
        "target_is_fixed": identity_lock.get("is_fixed") if identity_lock else False,
        "target_is_system_drive": (
            identity_lock.get("is_system_drive") if identity_lock else False
        ),
        "target_scan_source": (
            identity_lock.get("scan_source") if identity_lock else None
        ),
        ...
        "identity_lock_id": (
            identity_lock.get("identity_lock_id") if identity_lock else None
        ),
...
def validate_hardware_preflight_export_path(
    ...
    for suspicious in [
        ...
        if suspicious in p_str.replace("\\", "/"):
            raise ValueError(
                f"Suspicious path detected: export path in {suspicious} folders is blocked."
            )
    ...
    if target_drive:
        ...
        if (
            td_root
            and export_root
            and td_root.lower().rstrip("\\") == export_root.lower().rstrip("\\")
        ):
            raise ValueError(
...
def export_hardware_preflight_json(preflight_payload: dict, output_path: str) -> dict:
    ...
    try:
        validate_hardware_preflight_export_path(
            output_path, "json", preflight_payload.get("target_drive")
        )
...
def export_hardware_preflight_markdown(
    preflight_payload: dict, output_path: str
) -> dict:
    """
    ...
    try:
        validate_hardware_preflight_export_path(
            output_path, "markdown", preflight_payload.get("target_drive")
        )
...
def build_hardware_lab_permission_status(platform_name=None) -> dict:
    ...
    if blocked:
        block_reasons.append(
            "Administrative/root privileges are required to access physical drives."
        )
...
def build_physical_writer_dryrun_request(
    preflight_payload: dict, readiness_gate: dict = None, ledger_path: str = None
) -> dict:
    """
    ...
    target_stable_id = (
        preflight_payload.get("target_stable_id") if preflight_payload else None
    )
    target_identity_hash = (
        preflight_payload.get("target_identity_hash") if preflight_payload else None
    )
    ...
    image_size_bytes = (
        preflight_payload.get("image_size_bytes") if preflight_payload else 0
    )
    preflight_id = preflight_payload.get("preflight_id") if preflight_payload else None
    identity_lock_id = (
        preflight_payload.get("identity_lock_id") if preflight_payload else None
    )

    readiness_gate_id = (
        readiness_gate.get("readiness_gate_id") if readiness_gate else None
    )
    session_id = (
        (preflight_payload.get("session_id") if preflight_payload else None)
        or (readiness_gate.get("session_id") if readiness_gate else None)
        or f"session_{str(uuid.uuid4())[:32].replace('-', '')}"
    )
...
def validate_physical_writer_dryrun_request(request_payload: dict) -> tuple:
    ...
    if request_payload.get("schema") != "bootforge.physical_writer_dryrun_request.v1":
        block_reasons.append(
            "Invalid request schema. Must be bootforge.physical_writer_dryrun_request.v1."
        )
    ...
    if unlock != "I_ACCEPT_REAL_USB_WRITE_RISK":
        block_reasons.append(
            "Environment variable unlock 'BOOTFORGE_ENABLE_LAB_WRITE=I_ACCEPT_REAL_USB_WRITE_RISK' is missing."
        )

    # Check requested writes
    if request_payload.get("physical_write_requested") or request_payload.get(
...
def build_physical_writer_dryrun_result(
    request_payload: dict, validation_result: tuple = None
) -> dict:
    """
    ...
    if validation_result:
        ...
        is_valid, block_reasons = validate_physical_writer_dryrun_request(
            request_payload
        )
    ...
    return {
        ...
        "latest_identity_hash": request_payload.get(
            "target_identity_hash"
        ),  # matches in dry-run
        "identity_drift_detected": False,
    ...
    def execute_dryrun(self, request_payload: dict) -> dict:
        ...
        if target_drive:
            ...
            for d in drives:
                ...
                if d_path and d_path.lower().rstrip(
                    "\\"
                ) == target_drive.lower().rstrip("\\"):
                    details = d
                    break
            if details:
                is_fixed = details.get("is_fixed") or details.get("fixed")
                is_system_drive = details.get("is_system_drive") or details.get(
                    "system_drive"
                )
                if is_fixed or is_system_drive:
                    ...
                    if "Target drive is fixed/internal or system." not in block_reasons:
                        block_reasons.append(
                            "Target drive is fixed/internal or system."
                        )

        result = build_physical_writer_dryrun_result(
            request_payload, (is_valid, block_reasons)
        )
        return result
...
class WindowsPhysicalWriterAdapter:
    ...
    def execute_write(self, request):
        return {
            "blocked": True,
            "block_reasons": ["Windows physical writer is blocked."],
        }
...
def validate_physical_writer_dryrun_export_path(
    output_path: str, export_type: str, target_drive: str = None
):
    """
    ...
    if export_type == "json" and p_resolved.suffix.lower() != ".json":
        raise ValueError(
            f"Export path extension '{p_resolved.suffix}' must be '.json'."
        )
...
def export_physical_writer_dryrun_json(dryrun_payload: dict, output_path: str) -> dict:
    ...
    try:
        validate_physical_writer_dryrun_export_path(
            output_path, "json", dryrun_payload.get("target_drive")
        )
        with open(output_path, "w", encoding="utf-8") as f:
...
def export_physical_writer_dryrun_markdown(
    dryrun_payload: dict, output_path: str
) -> dict:
    """
    ...
    try:
        validate_physical_writer_dryrun_export_path(
            output_path, "markdown", dryrun_payload.get("target_drive")
        )
...
def build_physical_usb_write_lab_request(**kwargs) -> dict:
    req_id = (
        kwargs.get("request_id") or f"phyreq_{str(uuid.uuid4())[:32].replace('-', '')}"
    )
    created_at = kwargs.get("created_at") or datetime.now(timezone.utc).strftime(
        "%Y-%m-%dT%H:%M:%SZ"
    )
    image_size = kwargs.get("image_size_bytes") or 0
    chunk_size = kwargs.get("chunk_size_bytes") or 1048576
    expected_chunks = (
        (image_size + chunk_size - 1) // chunk_size if image_size > 0 else 0
    )
    ...
    return {
        ...
        "final_irreversible_acknowledgement": kwargs.get(
            "final_irreversible_acknowledgement"
        ),
...
def build_physical_usb_write_lab_result(**kwargs) -> dict:
    res_id = (
        kwargs.get("result_id") or f"phyres_{str(uuid.uuid4())[:32].replace('-', '')}"
    )
    created_at = kwargs.get("created_at") or datetime.now(timezone.utc).strftime(
        "%Y-%m-%dT%H:%M:%SZ"
    )
...
def build_physical_usb_write_lab_verification(**kwargs) -> dict:
    ver_id = (
        kwargs.get("verification_id")
        or f"phyver_{str(uuid.uuid4())[:32].replace('-', '')}"
    )
    created_at = kwargs.get("created_at") or datetime.now(timezone.utc).strftime(
        "%Y-%m-%dT%H:%M:%SZ"
    )
...
def validate_physical_usb_write_lab_gates(request: dict) -> tuple:
    ...
    if env_val != PHYSICAL_USB_WRITE_ENV_VALUE:
        block_reasons.append(
            "Environment variable BOOTFORGE_ENABLE_PHYSICAL_USB_WRITE is missing or wrong."
        )
    ...
    if not request.get("latest_identity_hash"):
        ...
        block_reasons.append(
            "Identity drift detected: target hash does not match latest re-scan hash."
        )
    ...
    if plat not in ("win32",):
        block_reasons.append(
            f"Physical USB writing is not implemented for platform '{plat}'."
        )
...
def validate_physical_usb_write_lab_export_path(
    output_path: str, export_type: str, target_drive: str = None
):
    from pathlib import Path
...
def export_physical_usb_write_lab_json(result_payload: dict, output_path: str) -> dict:
    ...
    try:
        validate_physical_usb_write_lab_export_path(
            output_path, "json", result_payload.get("target_drive")
        )
        with open(output_path, "w", encoding="utf-8") as f:
...
def export_physical_usb_write_lab_markdown(
    result_payload: dict, output_path: str
) -> dict:
    try:
        validate_physical_usb_write_lab_export_path(
            output_path, "markdown", result_payload.get("target_drive")
        )
...
def build_hardware_evidence_bundle(
    ...
    if target_drive:
        ...
        if resolved_count == 0:
            ...
            is_system = (
                resolved_device.get("is_system", False)
                or resolved_device.get("is_system_drive", False)
                or resolved_device.get("is_boot_drive", False)
            )
            ...
            try:
                dryrun_preview = {
                    ...
                    "identity_drift_detected": (
                        rescan_preview.get("drift_detected", False)
                        if rescan_preview
                        else False
                    ),
                    ...
                    + (
                        identity_lock_preview.get("block_reasons", [])
                        if identity_lock_preview
                        else []
                    ),
                }
            except Exception:
    ...
    if redact_serials:
        ...
        if scanner_stable_id:
            scanner_stable_id = hashlib.sha256(scanner_stable_id.encode()).hexdigest()[
                :16
            ]
    ...
    bundle = {
        ...
        "next_required_action": (
            "await_phase_5c_windows_writer" if eligible else "resolve_block_reasons"
        ),
    }
...
def generate_hardware_evidence_markdown(bundle: dict) -> str:
    ...
    warnings_str = (
        "\n".join(f"- {w}" for w in bundle.get("scanner_warnings", []))
        if bundle.get("scanner_warnings")
        else "None"
    )
...
def validate_hardware_evidence_export_path(
    output_path: str, export_type: str, target_drive: str = None
):
    from pathlib import Path
    ...
    if target_drive:
        try:
            ...
            if td_root:
                ...
                if p_resolved == td_path:
                    raise ValueError(
                        "Export path cannot be the target drive root itself."
                    )
        except ImportError:
            pass
