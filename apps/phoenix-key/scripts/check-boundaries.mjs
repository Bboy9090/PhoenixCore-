...
if (!rust.includes("DeviceFamily::IPhone") || !rust.includes("DeviceFamily::AndroidPhone")) failures.push("mobile-device family allowlist is missing");
if (!rust.includes("DeviceMode::Recovery") || !rust.includes("DeviceMode::Fastboot") || !rust.includes("DeviceMode::Dfu")) failures.push("recovery and service-mode allowlist is missing");
