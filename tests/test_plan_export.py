...
class TestPlanExport(unittest.TestCase):
    ...
    def test_export_success_json_and_markdown(self, mock_drive_safety, mock_image_inspect):
        ...
        json_res = usb_creator.build_audit_export_payload("E:\\", "debian.iso", "json", json_path)
        ...
        md_res = usb_creator.build_audit_export_payload("E:\\", "debian.iso", "markdown", md_path)
        ...
        with open(md_path, "r", encoding="utf-8") as f:
            ...
            self.assertIn("This report is evidence of a dry-run audit only.", md_content)
            self.assertIn("It does not indicate that a write, format, partition, or mount operation was performed.", md_content)

    @patch("usb_creator.build_image_inspection_payload")
    @patch("usb_creator.build_drive_safety_payload")
    def test_export_blocks_if_output_exists(self, mock_drive_safety, mock_image_inspect):
    ...
    @patch("usb_creator.build_image_inspection_payload")
    @patch("usb_creator.build_drive_safety_payload")
    def test_export_blocks_if_on_target_drive(self, mock_drive_safety, mock_image_inspect):
        ...
        with patch("usb_creator.get_drive_root") as mock_root:
            mock_root.side_effect = lambda path: "E:\\" if "E:" in str(path) or "audit" in str(path) else "/"
    ...
    @patch("usb_creator.build_image_inspection_payload")
    @patch("usb_creator.build_drive_safety_payload")
    def test_export_blocks_if_extension_mismatch(self, mock_drive_safety, mock_image_inspect):
        ...
        json_res = usb_creator.build_audit_export_payload("E:\\", "debian.iso", "json", bad_json_path)
        self.assertEqual("failed", json_res["status"])
        ...
        md_res = usb_creator.build_audit_export_payload("E:\\", "debian.iso", "markdown", bad_md_path)
        self.assertEqual("failed", md_res["status"])
        self.assertIn("does not match format 'markdown'", md_res["error"])

    @patch("usb_creator.build_image_inspection_payload")
    @patch("usb_creator.build_drive_safety_payload")
    def test_failed_audit_still_exports_correctly(self, mock_drive_safety, mock_image_inspect):
        ...
        with patch("usb_creator.get_drive_root") as mock_root:
            mock_root.side_effect = lambda path: "C:\\" if str(path) == "C:\\" else "T:\\"

            # Perform JSON export for failed plan
            json_res = usb_creator.build_audit_export_payload("C:\\", "nonexistent.iso", "json", json_path)
            self.assertEqual("success", json_res["status"])
            ...
            md_res = usb_creator.build_audit_export_payload("C:\\", "nonexistent.iso", "markdown", md_path)
            self.assertEqual("success", md_res["status"])
    ...
    @patch("usb_creator.build_image_inspection_payload")
    @patch("usb_creator.build_drive_safety_payload")
    def test_export_blocks_if_parent_directory_missing(self, mock_drive_safety, mock_image_inspect):
        ...
        json_res = usb_creator.build_audit_export_payload("E:\\", "debian.iso", "json", bad_path)
