...
def audit(ok=True, size=3145728):
    return {
        ...
        "plan_id": "bootforge-plan-test",
        "plan_hash": "hash",
        "eligible": ok,
        "blocked": not ok,
        "block_reasons": [] if ok else ["System drive blocked."],
        "checks": [],
        "write_plan": {
            "target_drive": "E:\\",
            "image_path": "test.img",
...
class TestMockWriter(unittest.TestCase):
    ...
    def test_success_progress_and_shape(self, m):
        ...
        p = usb_creator.build_mock_writer_payload(
            "E:\\", "test.img", chunk_size=1024 * 1024
        )
    ...
    def test_failure_and_cancel_injection(self, m):
        m.return_value = audit(True, 3 * 1024 * 1024)
        f = usb_creator.build_mock_writer_payload(
            "E:\\", "test.img", chunk_size=1024 * 1024, fail_at_chunk=2
        )
        c = usb_creator.build_mock_writer_payload(
            "E:\\", "test.img", chunk_size=1024 * 1024, cancel_at_chunk=2
        )
