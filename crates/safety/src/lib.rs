...
pub fn can_write_to_disk(ctx: &SafetyContext, is_system_disk: bool) -> SafetyDecision {
    ...
    match &ctx.confirmation_token {
        ...
        None => SafetyDecision::Deny("Confirmation token required for destructive operations.".to_string()),
    }
}
...
mod tests {
    ...
    fn test_safety_policy() {
        let ctx_none = SafetyContext { force_mode: false, confirmation_token: None };
        assert!(matches!(can_write_to_disk(&ctx_none, false), SafetyDecision::Deny(_)));

        let token = require_confirmation_token();
        let ctx_valid = SafetyContext { force_mode: false, confirmation_token: Some(token) };
        assert_eq!(can_write_to_disk(&ctx_valid, false), SafetyDecision::Allow);

        // System disk block
        assert!(matches!(can_write_to_disk(&ctx_valid, true), SafetyDecision::Deny(_)));
        
        let ctx_force = SafetyContext { force_mode: true, confirmation_token: ctx_valid.confirmation_token };
