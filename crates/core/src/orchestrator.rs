...
impl Orchestrator {
    pub fn new() -> Self {
        Self { 
            ...
            downloader: crate::downloader::PayloadDownloader::new(std::path::PathBuf::from("./bin")),
        }
    }
    ...
    pub fn execute_job(&self, job_json: &str) -> Result<()> {
        ...
        self.matrix.enforce_gate(&job, &current_state, actual_disk_size)?;
        thread::sleep(Duration::from_millis(500));
        
        if dry_run {
            self.stream_status("DRY_RUN", "🛡 DRY RUN ACTIVE: Hardware fingerprint verified. No bits flipped.");
        }

        // 1. Capability Gating
        self.stream_status("CAPABILITY_CHECK", "Verifying target device compatibility...");
        thread::sleep(Duration::from_millis(500));
        ...
        for i in (0..=100).step_by(10) {
            if force_error && i == 50 {
                return Err(anyhow::anyhow!("SIMULATED_FAILURE: Transactional rollback triggered at 50%"));
            }
        ...
        // 4. Result
        println!(r#"{{"type": "result", "success": true, "job_id": "{}"}}"#, job_id);

        Ok(())
    }

    fn stream_status(&self, stage: &str, message: &str) {
        println!(r#"{{"type": "status", "stage": "{}", "message": "{}"}}"#, stage, message);
    }
